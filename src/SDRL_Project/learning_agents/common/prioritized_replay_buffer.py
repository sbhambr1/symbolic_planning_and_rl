...
class PrioritizedReplayBuffer(ReplayBuffer):
    ...
    def load_demo(self, demo):
        super(PrioritizedReplayBuffer, self).load_demo(demo)
        # for init priority of demo
        self.tree_idx = self.demo_size
        for i in range(self.demo_size):
            self.sum_tree[i] = self._max_priority ** self.alpha
    ...
    def sample(self, beta=0.4, is_to_tensor=True, indices=None):
        """
        ...
        if indices is not None:
            return super(PrioritizedReplayBuffer, self).sample(indices=indices, is_to_tensor=is_to_tensor)
