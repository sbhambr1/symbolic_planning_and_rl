...
class MetaNN:
    def __init__(self):
        ...
        with tf.device('/{0}:'.format(device) + str(GPU)):
            self.meta_controller = Sequential()
            self.meta_controller.add(
            ...
            self.meta_controller.add(Dense(HIDDEN_NODES, activation='relu',
    ...
    def train(self):
        ...
        self._train_y = np.reshape([self._selected_replay_data[i][1] for i in xrange(BATCH_SIZE)],
                                   (BATCH_SIZE, nb_Option))

        self.meta_controller.fit(self._train_x, self._train_y, batch_size=32, epochs=self.num_pass,
